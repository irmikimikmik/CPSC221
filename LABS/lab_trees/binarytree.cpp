 ...
 */
template <typename T>
void BinaryTree<T>::printPaths(const Node* subRoot, string x) const
{
    if (subRoot != NULL){
        x += to_string(subRoot->elem);
        if (subRoot->left == NULL && subRoot->right == NULL)
        {
            cout << "Path: " + x << endl;
        }
        x += " ";
        printPaths(subRoot->left, x);
        printPaths(subRoot->right, x);
    }
...
/**
 * Private helper function for the sumDistances function, with an additional
 * Node* parameter to allow for recursive calls.
 * @param subRoot
 * @return The sum of the distances of all nodes to the root.
 */
template <typename T>
int BinaryTree<T>::sumDistances(const Node* subRoot, int x) const
{
    if (subRoot != NULL) {
        x++;
        return x - 1 + sumDistances(subRoot->left, x) + sumDistances(subRoot->right, x);
    } else {
        return 0;
    }
...
/**
 * Private helper function for the isOrdered function, with an additional
 * @param subRoot
 * @return True if an in-order traversal of the tree would produce a
 *  nondecreasing list output values, and false otherwise. This is also the
 *  criterion for a binary tree to be a binary search tree.
